...
fn newSymbolAssumeCapacity(self: *InternalObject, name_off: u32, elf_file: *Elf) Symbol.Index {
    ...
    symbol.name = name_off;
    symbol.extra = self.addSymbolExtraAssumeCapacity(.{});
    symbol.ref = .{ .index = 0, .file = 0 };
    symbol.esym_idx = esym_index;
    symbol.ver_idx = elf_file.default_sym_version;
    return index;
}

pub fn initSymbols(self: *InternalObject, elf_file: *Elf) !void {
    const gpa = elf_file.base.allocator;

    var nsyms: usize = 0;
    if (elf_file.getEntryName()) |_| {
        nsyms += 1; // entry
    }
    nsyms += 1; // _DYNAMIC
    ...
    nsyms += 1; // _end
    if (elf_file.options.eh_frame_hdr) {
...
pub fn initStartStopSymbols(self: *InternalObject, elf_file: *Elf) !void {
    const gpa = elf_file.base.allocator;
...
pub fn allocateSymbols(self: *InternalObject, elf_file: *Elf) void {
    ...
    if (elf_file.getSectionByName(".preinit_array")) |shndx| {
        const shdr = shdrs[shndx];
    ...
    if (self.global_pointer_index) |index| {
        const value, const osec = if (elf_file.getSectionByName(".sdata")) |shndx| .{
...
pub fn getDynamicSymbol(self: InternalObject, elf_file: *Elf) ?*Symbol {
    const index = self.dynamic_index orelse return null;
    const resolv = self.resolveSymbol(index, elf_file);
    return elf_file.getSymbol(resolv);
}

pub fn getEntrySymbol(self: InternalObject, elf_file: *Elf) ?*Symbol {
    const index = self.entry_index orelse return null;
    const resolv = self.resolveSymbol(index, elf_file);
    return elf_file.getSymbol(resolv);
}

fn addString(self: *InternalObject, allocator: Allocator, str: []const u8) !u32 {
...
pub fn resolveSymbol(self: InternalObject, index: Symbol.Index, elf_file: *Elf) Elf.Ref {
